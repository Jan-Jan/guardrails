...
| `UNANCHORED-DEF …` (check-ids) | **Not a violation, and never fails the run.** A `**ID**:` written somewhere other than the start of a line, numbered above the highest ID actually defined. It defines nothing, but it used to raise the number `finalize-ids.sh` mints from, so the number it was reserving is now free | Usually nothing — it is prose about an item. If it was *meant* as a definition, move it to the start of its line in the document its prefix belongs to. If a later change re-mints that number, this line is the warning you were given. |
| `UNANCHORED-DEF-UNREADABLE` (check-ids) | **Not a violation.** A file the off-column scan must read has a newline in its path, which its framing cannot represent, so that one scan did not run | Rename the file. Every other gate in that run is unaffected. |
