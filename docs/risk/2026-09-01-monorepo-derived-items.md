...
contain descriptive labels, not minted HAZ/RC tokens — token minting follows
...
limits are unset, and the item stays indefinitely as one printed line per run —
...
- An `expects:` naming a unit absent from the unit's `depends_on:` convicts
  ...
  containing `expects: <unit>` with `<unit>` not in `depends_on:` convicts and
...
is correct by design at every link — the hazard is that `check-units.sh` gets
...
- **export-removal-reopens-expectation**: the removed export contained
...
bind on the unit machinery's bats suite wherever each conviction occurs; no
