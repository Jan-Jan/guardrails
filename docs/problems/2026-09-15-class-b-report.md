...
was touched. The report carried eight points; these are the four that name a
...
`finish-merge.sh` offers no way to prove it, so a refusal is discovered after
...
protocol creates the condition guard 4 refuses.
...
which the rest of the script refuses — and removes nothing; the scan is factored
...
before any squash`, `--check refuses the base branch`, `--check never removes,
...
carry — it keeps aging toward `STALE-PROBLEM` and keeps counting against
...
gap — this ledger already carries the defect the report describes, three times
...
with the keyword and a wrap that lands it at column one is read as a real
...
the kind skills/ratchet already carries for a tightening — not the report's
...
independent review found this line still carrying the superseded 41-era
...
first count here was 41, of which this change was said to have caused three.
...
of truth for the check scripts and says nothing about how a skill reaches the
...
`~/.claude/skills/<name>` symlinks to `~/.agents/skills/<name>`, which holds
...
this change, carrying edits from e2eac86 and 51f01b7 that never reached the
...
* **Five skills, not two, had already diverged from `main` before this change
  ...
  The first draft said two, which understated the standing drift by more than
...
* **The cause is not measured, and `install.sh --copy` is a guess.** Neither of
  ...
  also holds skills from elsewhere (`solidity`, `find-skills`, `eli5`). That is
...
`finish-merge.sh --check <branch>`. The preflight ran anyway because the author
...
which is exactly the defect PR-k77dzn exists to close. A skill that says one
...
carrying the same divergence in its own skills while reading that praise.
...
whether `--copy` should survive if it turns out to be the mechanism; and whether
...
were caught by hand, and M03 and M38 survived into the independent review — after
