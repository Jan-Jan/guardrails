...
points to, and is then denied every git operation, every write through the
...
subagent to its dispatcher's worktree, rejects every operation against any
...
denied every git operation, every write, and every shell call after the
...
contains the 2026-08-31 measurement of both locations — scoped to the one
harness it was taken on, with a probe a project can run to tell whether its
own harness pins — defines `<N>`, and states how a subagent gets inside what it
...
`ratchet`'s gitignore step contain the same rule, and `.gitignore` covers the
...
duplicate the base merge left behind, and pin the entry with a test.

Nesting created a hazard of its own, and the only code in this change closes
it. `git worktree remove` does not reject a change worktree that has a
...
findings never reaches a later step — so the guard does not reject the
...
cover prose or behavior that already existed and that nothing asserted, so they
...
contains no count of the rounds. The round that reads it is always the
